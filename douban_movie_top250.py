...
def parse_html(html, movies):
    ...
    for movie_li in movie_list_soup.find_all('li'):
        pic = movie_li.find('div', attrs={'class': 'pic'}).find(
            'a').find('img')['src']
        ...
        quote_div = detail.find('div', attrs={'class': 'bd'}).find(
            'p', attrs={'class': 'quote'})
        quote = ""
        if quote_div:
...
def bt_login(moviename):
    ...
    resp = s.post('http://www.bttt99.com/e/search/',
    ...
    try:
        ...
        if linkId:
            ...
            for magnet in magnet_links:
                lk = magnet.find('a')
                if lk:
                    title = magnet.find('a')['title'].replace(
                        moviename, "").strip()
